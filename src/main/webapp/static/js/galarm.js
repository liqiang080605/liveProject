$(function() {
			...
			.DataTable(
					{
						...
						"ajax" : "/galarm/list/" + $("#gid").val(),
						"ordering" : false,
						"dom" : '<"toolbar"><"col-sm-6"l>rt<"col-sm-6"i><"col-sm-6"p>',
						// id, ip, idc, model, power_u, power_p, update_time,
						// create_time
						"columns" : [
								{
									"data" : "id",
	...
	$("div.toolbar")
			/*
			 * .append('<button class="btn btn-flat bg-blue margin"
			 * id="goback">返回分组列表</button>')
			 */
			.append(
					'<button class="btn btn-flat bg-blue margin" id="create">添加</button>')
			.append(
					'<button class="btn btn-flat bg-blue margin" id="modify">修改</button>')
			// .append('<button class="btn btn-flat bg-blue margin"
			// id="batch">批量导入</button>')
			.append(
	...
	$("#modify")
			.click(
					function() {
						...
						var group_id = $("#table").dataTable().api().row(
								".selected").data().group_id;
						var user_id = $("#table").dataTable().api().row(
						...
						var modify_tmpl = [
								...
								'                   <input type="hidden" id="gid" name="gid" value="" /><button type="button" class="btn btn-primary">确认</button>                         ',
								'               </div>                                                                                                                                   ',
								'               <div class="col-sm-6">                                                                                                                   ',
						...
						if (meter_name == 'disk.io.read.bytes.rate'
								|| meter_name == 'disk.io.write.bytes.rate') {
						...
						$tmpl = $tmpl.find("#alarm_level").val(alarm_level)
								.end();
						$tmpl = $tmpl.find("#gid").val(group_id).end();
						...
						//
						// $tmpl = $(modify_tmpl).find(
						// "select[id='comparison_operator']").val(
						// comparison_operator).end();
	...
	/*
	 * $("#goback").click(function(){ window.location = "/group";
	 * 
	 * });
	 */
